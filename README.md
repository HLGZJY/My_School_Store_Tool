...
- **UI组件库**：uni-ui
- **状态管理**：Vuex
- **后端服务**：DCloud云开发（uniCloud 云函数 + MongoDB）
- **AI服务**：Moonshot API

## 架构概览

```
多源信息（教务处/图书馆/学院官网/公众号等）
        │
        ▼
数据采集（定时爬取 + URL 队列管理）
        │
        ▼
内容解析（规则解析 + AI 智能解析：标题/正文/标签/摘要）
        │
        ▼
uniCloud 云函数（文章/用户/收藏/订阅/推荐 等 30+ 云函数）
        │
        ▼
uni-app 前端（信息流 / 发现 / 详情 / 我的 / 管理员后台）
```

- **三角色个性化推荐**：学生/教师/行政人员按角色推荐相关资讯
- **AI 赋能**：智能提取标签、生成摘要、文章自动分类
...
4. 配置环境变量
在云开发控制台配置（敏感信息务必通过环境变量配置，不要硬编码到代码中）：
```
