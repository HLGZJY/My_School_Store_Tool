...
export default {
    ...
    methods: {
        ...
        async loadSources() {
            try {
                const result = await userApi.getSubscribeSources();
                // getSubscribeSources 返回 { sources: [...] }，api.call 返回其 data 字段
        ...
        async loadHotKeywords() {
            const data = await loadWithCache('search_hot', async () => {
                const result = await userApi.getHotKeywords({ limit: 10 })
                return result || []
            })
