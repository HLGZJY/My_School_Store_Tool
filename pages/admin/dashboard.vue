...
export default {
    ...
    methods: {
        ...
        async loadStats() {
            try {
                // api.call returns res.result.data on success, throws on error
                const stats = await adminApi.adminStats('getStats')

                console.log('[adminStats] 响应:', JSON.stringify(stats))

                // Success - api.call throws on error, so if we get here data is valid
        ...
        async loadLogs() {
            try {
                // api.call returns res.result.data on success, throws on error
                const logs = await adminApi.adminStats('getLogs')
