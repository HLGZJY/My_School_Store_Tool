...
| setUserRole | 设置用户角色 | 客户端调用 |
| updateUserSettings | 更新用户设置 | 客户端调用 |
| toggleSubscribe | 切换订阅源 | 客户端调用 |
| getArticles | 获取文章列表 | 客户端调用 |
| getArticleDetail | 获取文章详情 | 客户端调用 |
| searchArticles | 搜索文章 | 客户端调用 |
| recordRead | 记录阅读 | 客户端调用 |
| collectArticle | 收藏/取消收藏 | 客户端调用 |
| getCollections | 获取收藏列表 | 客户端调用 |
| batchUncollect | 批量取消收藏 | 客户端调用 |
| getReadingHistory | 获取阅读历史 | 客户端调用 |
| clearReadingHistory | 清空阅读历史 | 客户端调用 |
| getMessages | 获取消息列表 | 客户端调用 |
| markMessageRead | 标记消息已读 | 客户端调用 |
| getHotRanking | 获取热门排行 | 客户端调用 |
| getTagCloud | 获取标签云 | 客户端调用 |
| getTimeline | 获取时间轴 | 客户端调用 |
| getSubscribeSources | 获取订阅源列表 | 客户端调用 |
| getSystemConfig | 获取系统配置 | 客户端调用 |
| reportSearch | 上报搜索记录 | 客户端调用 |
| syncData | 数据同步 | 定时触发 |
| processArticle | AI处理文章 | 云函数调用 |
| getReviewQueue | 获取待审核队列 | 管理后台调用 |
| reviewArticle | 审核文章 | 管理后台调用 |
| initDatabase | 初始化数据库 | 手动调用 |
| cleanupExpiredData | 清理过期数据 | 定时触发 |
| updateHotKeywords | 更新热门搜索 | 定时触发 |
...
exports.main = async (event, context) => {
    const { userId, category, page = 1, pageSize = 15, refresh = false } = event;
    const db = uniCloud.database();
    ...
    try {
        ...
        if (user && user.role) {
            ...
            whereCondition['tags.role'] = $.in([roleTag, '通用']);
        }

        // 订阅源筛选
        if (user && user.subscribeSources && user.subscribeSources.length > 0) {
            whereCondition.sourceId = $.in(user.subscribeSources);
        }
