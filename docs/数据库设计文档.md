...
| 集合 | 索引名称 | 字段 | 类型 | 说明 |
|-----|---------|------|------|------|
| users | idx_openid | openid | 唯一 | 微信OpenID |
...
| collections | idx_userId | userId | 普通 | 按用户 |
| collections | idx_articleId | articleId | 普通 | 按文章 |
| collections | idx_user_time | [userId, createTime] | 复合 | 用户收藏 |
| collections | idx_user_article | [userId, articleId] | 唯一 | 防重复 |
| reading_history | idx_userId | userId | 普通 | 按用户 |
| reading_history | idx_user_time | [userId, readTime] | 复合 | 用户历史 |
| reading_history | idx_readTime_ttl | readTime | TTL | 30天过期 |
| search_history | idx_userId | userId | 普通 | 按用户 |
...
| messages | idx_userId | userId | 普通 | 按用户 |
| messages | idx_isRead | isRead | 普通 | 按状态 |
| messages | idx_user_time | [userId, createTime] | 复合 | 用户消息 |
...
| operation_logs | idx_userId | userId | 普通 | 按用户 |
